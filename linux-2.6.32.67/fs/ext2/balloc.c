...
//假设块大小1024 则每块的desc数目 
	group_desc = block_group >> EXT2_DESC_PER_BLOCK_BITS(sb);//意思是块组号0-31的块组的描述符都在0 ，32-63的都在1
	offset = block_group & (EXT2_DESC_PER_BLOCK(sb) - 1);//偏移量 
	//上面这两行的意思大致是:block_group=0~31 group_desc=0,offset=0~31
...
//加上下面这行，意思大致是:block_group=0~31的  因为group_desc=0 ，所以desc指向s_group_desc[0]->b_data(b_data是缓冲区的数据，也就是一个数据块1024字节大小的缓冲区，里面是32个块组的块组描述符)
	desc = (struct ext2_group_desc *) sbi->s_group_desc[group_desc]->b_data;
	if (bh)
		*bh = sbi->s_group_desc[group_desc];
	return desc + offset;//desc已经指向含有我们要找的那个块组的块组描述符那个缓冲区快了
	//offset是该块组的块组描述符在该块中的偏移量
}
...
void ext2_init_block_alloc_info(struct inode *inode)
{
	...
	if (block_i) {
		...
		if (!test_opt(sb, RESERVATION))
			...
			rsv->rsv_goal_size = EXT2_DEFAULT_RESERVE_BLOCKS;//8
		rsv->rsv_alloc_hit = 0;
...
do_more:
	...
	brelse(bitmap_bh);
	bitmap_bh = read_block_bitmap(sb, block_group);//读取块组块位图
	if (!bitmap_bh)
		goto error_return;

	desc = ext2_get_group_desc (sb, block_group, &bh2);//获取块组块组描述符
	if (!desc)
		goto error_return;
...
retry_alloc:
	 ...
	 */
	 //如果块组没有足够的块给新的预留窗口 关掉预留
	if (my_rsv && (free_blocks < windowsz)
		&& (free_blocks > 0)
		&& (rsv_is_empty(&my_rsv->rsv_window)))
		my_rsv = NULL;
//如果有空闲数据块 获取该目标块在该块组的块偏移量
	if (free_blocks > 0) {
